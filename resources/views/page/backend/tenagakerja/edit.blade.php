...
<div class="content-wrapper d-flex align-items-center justify-content-center" style="min-height: 100vh;">
  <div class="col-md-8">
    <div class="card shadow-lg rounded-3">
      <div class="card-body">
        <h3 class="mb-4 fw-bold text-center">Edit Tenaga Kerja</h3>
        ...
        <form action="{{ route('tenagakerja.update', $tenagakerja->id) }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="mb-3">
            ...
            <textarea name="description" class="form-control" rows="4" required>{{ $tenagakerja->description }}</textarea>
          </div>
          <div class="mb-3">
            <label class="form-label fw-bold">Current Photo</label><br>
            @if($tenagakerja->photo && Storage::disk('public')->exists($tenagakerja->photo))
              <img src="{{ asset('storage/'.$tenagakerja->photo) }}" width="120" class="rounded mb-2" style="object-fit:cover;">
            @else
